...
Future<bool> createServiceRequest(
  ...
  try {
    ...
    QuerySnapshot snapshot = await firestore.collection('ServiceReport').get();
    int numDocuments = snapshot.docs.length;

    // Generate a unique sector ID
    int final_id = numDocuments + 1;
    String report_id = "Service_Report_" + final_id.toString();
    // Create a new document in the Sectors collection
    await firestore.collection('ServiceReport').doc(report_id).set({
      "createdBy": createdBy,
    ...
    print('Service Report created successfully!');
    return true;
  } catch (e) {
    print('Error creating Service Report: $e');
    return false;
  }
}
